...
export class CdkStarterStack extends cdk.Stack {
  constructor(scope: cdk.App, id: string, props?: cdk.StackProps) {
    ...
    const DetailTodosLambda = new lambda.Function(this, 'detail-todos-lambda', {
      runtime: lambda.Runtime.NODEJS_16_X,
      handler: 'index.main',
      code: lambda.Code.fromAsset(path.join(__dirname, '/../src/detail-todo')),
    });

    // 👇 add a /todos resource
    const todos = api.root.addResource('todos');
    const todosdetail = todos.addResource('{todoId}');
    ...
    todosdetail.addMethod(
      'GET',
      new apigateway.LambdaIntegration(DetailTodosLambda, {proxy: true}),
    );
